...
from app.infrastructure.db.models import RiskSnapshotRow
...
class SqlAlchemyRiskSnapshotRepository:
    ...
    def get(self, snapshot_id: str) -> RiskSnapshotRecord | None:
        try:
            ...
            return None
        with self._sessions() as session:
            row = session.get(RiskSnapshotRow, identifier)
        ...
        *,
        ...
        if job_id is not None:
            try:
                ...
                return (), 0
            filters.append(RiskSnapshotRow.job_id == identifier)
        if risk_level is not None:
            filters.append(RiskSnapshotRow.risk_level == risk_level)
        if from_time is not None:
            filters.append(RiskSnapshotRow.created_at >= from_time)
        if to_time is not None:
            filters.append(RiskSnapshotRow.created_at <= to_time)
        with self._sessions() as session:
            total = session.scalar(
                select(func.count()).select_from(RiskSnapshotRow).where(*filters)
            )
            rows = session.scalars(
                select(RiskSnapshotRow)
                .where(*filters)
                .order_by(RiskSnapshotRow.created_at.desc())
                .limit(limit)
                .offset(offset)
            ).all()
...
def _row_to_record(row: RiskSnapshotRow) -> RiskSnapshotRecord:
